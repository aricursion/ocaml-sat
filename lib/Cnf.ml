...
let from_dimacs filename =
  ...
  let lines = List.map (fun s -> String.sub s 0 (String.length s - 2)) lines in
...
let to_dimacs (cnf : t) =
  ...
  let rec disj_to_string (disj : Clause.t) =
    ...
    | x :: xs -> (
        match x with
        | true, s -> string_of_int (Hashtbl.find ht s) ^ " " ^ disj_to_string xs
        | false, s ->
...
let stats cnf =
  { num_vars = Set.cardinal cnf.vars; num_clauses = cnf.num_clauses }
